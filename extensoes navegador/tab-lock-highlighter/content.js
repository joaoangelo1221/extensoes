(function () {
  ...
  async function submitUnlock() {
    ...
    const res = await safeRuntimeSend({ type: "TRY_UNLOCK_FROM_CONTENT", tabId: tabState.tabId, password });
    if (res?.ok) {
      clearLockVisuals();
      return;
    }
    ...
    (async () => {
      switch (msg?.type) {
        case "HIGHLIGHT_SELECTION":
          sendResponse?.({ ok: highlightSelection(msg.color || "#fff59d") });
          return;
        case "CLEAR_HIGHLIGHTS":
          ...
          return;
        case "ENABLE_HIGHLIGHT_REMOVE_MODE":
        ...
        case "CLEAR_NOTES":
          ...
          return;
        case "APPLY_LOCK_VISUALS":
          await applyLockVisuals(msg.amount);
          sendResponse?.({ ok: true });
          return;
        case "CLEAR_LOCK_VISUALS":
          clearLockVisuals();
          sendResponse?.({ ok: true });
          return;
  ...
  safeRuntimeSend({ type: "GET_TAB_STATE" }).then((state) => {
    if (!state) return;
    applySettings(state);
